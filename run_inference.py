...
if __name__ == '__main__':
    ...
    parser.add_argument("--save_path", type=str, default=None,
                        help="Path to save the output .ply file. If not provided, point cloud will not be saved.")
    parser.add_argument("--camera_poses_path", type=str, default=None,
                        help="Path to save the camera poses to a .npz file.")
    ...
    # 5. Save camera poses (optional)
    if args.camera_poses_path:
        print(f"Saving camera poses to: {args.camera_poses_path}")
        camera_poses = res['camera_poses'][0].cpu().numpy()
        np.savez(args.camera_poses_path, camera_poses=camera_poses)
