...
def main():
    """
    Main function to process the sliced videos for Task 2 - Memory-optimized version.
    Uses in-memory video slicing to avoid GPU memory overflow instead of creating temporary files.
    """
    # --- Add argument parser for parallel processing ---
    parser = argparse.ArgumentParser(description="Process sliced videos in parallel with memory optimization.")
    ...
    print(f"Found {len(videos_to_process)} videos from list file to process for Task 2 (memory-optimized).")
    ...
    for i, video_name in enumerate(videos_for_this_process):
        ...
        # --- Process each of the 3 slices directly with memory-optimized loading ---
        for slice_num in [1, 2, 3]:
            ...
            # Use the modified run_inference.py with --slice_part parameter for memory-efficient processing
            command = [
                ...
                "--device", device_name,
                "--slice_part", str(slice_num)  # Memory-optimized parameter to slice the video
            ]
