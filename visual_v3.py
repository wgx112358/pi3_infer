...
PLOT_OUTPUT_DIR = "output/comparison_plots_v3"
...
def main():
    ...
    for i, basename in enumerate(video_basenames):
        ...
        # --- Step 2: External Alignment (Independent Alignment to MegaSAM) ---
        # After internal alignment of the starting poses, we align each trajectory to the 
        # MegaSAM reference independently. This is necessary because the stitching process
        # introduces significant and inconsistent scale errors into the sliced trajectory,
        # making its scale not directly comparable to the full trajectory's scale.
        # This approach ensures both are fairly scaled against the ground truth for comparison.

        # Align the full trajectory to MegaSAM
        s_full, R_full, t_full = compute_umeyama_transform(poses_full, poses_megasam)
        pos_full_aligned = apply_transform(poses_full, s_full, R_full, t_full)

        # Independently align the (internally-aligned) stitched trajectory to MegaSAM
        s_stitched, R_stitched, t_stitched = compute_umeyama_transform(poses_stitched_in_full_system, poses_megasam)
        pos_stitched_aligned = apply_transform(poses_stitched_in_full_system, s_stitched, R_stitched, t_stitched)
